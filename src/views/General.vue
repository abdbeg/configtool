<template>
	<b-container>
		<b-card header="General Preferences">
			<b-form-row>
				<b-col>
					<b-form-group label="Board:" disabled>
						<b-form-select :value="template.board" @change="setSelectedBoard($event)" v-preset="preset.board" title="Board on which the firmware runs">
							<option value="evnoia">Evnoia</option>
			...
			<b-form-checkbox v-if="template.board === 'evnoia' || template.board === 'biquskr_1.3' || template.board === 'biquskr_1.4' || template.board === 'mkssgenl_1.0'" v-model="standalone" v-preset.left title="Run RepRapFirmware in stand-alone mode without an attached single-board computer">Run in standalone mode without SBC</b-form-checkbox>
